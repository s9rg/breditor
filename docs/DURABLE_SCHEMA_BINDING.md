...
`0.3.0-alpha.17` checkpoint. Wasm ABI 5 and the browser explicitly select
...
Alpha.10's Clear Formatting command does not change the content language, so it
does not enter the schema fingerprint. Its forward and inverse history use the
...
- ABI 5 retains Profile Bootstrap V2 and exposes the exact typed property declaration and
  ...
  and record. Alpha.8's unset/uniform/mixed complete-map observation and all
  hydrated or dirty browser draft state are excluded too; see
