...
/// Stable namespaced identity of one editor action.
#[derive(Clone, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct ActionId(QualifiedName);

impl ActionId {
    /// Validates and creates an action identity.
    ///
    /// # Errors
    ///
    ...
    /// Creates an action identity from an already validated qualified name.
    #[must_use]
    ...
    /// Returns the complete namespaced action name.
    #[must_use]
...
impl fmt::Debug for ActionId {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.debug_tuple("ActionId").field(&self.as_str()).finish()
    }
}
...
impl From<QualifiedName> for ActionId {
