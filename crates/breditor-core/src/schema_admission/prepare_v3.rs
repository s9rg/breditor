...
pub(super) fn prepare(
    ...
    let target_document = source_state
        ...
        .map_err(|error| SchemaAdmissionV3Error::Document(Box::new(error)))?;
    ...
    .map_err(|error| SchemaAdmissionV3Error::TargetState(Box::new(error)))?;
    let target_session =
    ...
    .map_err(|_| SchemaAdmissionV3Error::CheckpointInvariant)?;
    ...
    let target_checkpoint_json = checkpoint_codec
        ...
        .map_err(|error| SchemaAdmissionV3Error::CheckpointEncoding(Box::new(error)))?;

    Ok(PreparedSchemaAdmissionV3 {
