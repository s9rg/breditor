...
#[path = "typed_tests_v3.rs"]
mod typed;

use crate::{
    codec::{
        LOCAL_LOG_CHECKPOINT_FORMAT, LocalLogCheckpointJsonCodecV3, LocalLogCheckpointLimits,
        LocalLogFrameLimits, LocalLogStorageRootBinding, LocalLogStorageRootJsonCodecV3,
...
use super::{SchemaAdmissionErrorCode, SchemaAdmissionRequest, SchemaAdmissionV3Error};
...
fn compiled_extension_profile_is_a_real_structural_admission_target() -> TestResult {
    ...
    let prepared = request.try_prepare_v3(&source)?;
    ...
    let codec = LocalLogCheckpointJsonCodecV3::new(
...
#[test]
fn success_preserves_ast_resets_history_and_emits_one_canonical_v3_checkpoint() -> TestResult {
    ...
    assert!(prepared.target_checkpoint_json().starts_with(&format!(
        "{{\"format\":\"{LOCAL_LOG_CHECKPOINT_FORMAT}\",\"formatVersion\":3"
    )));

    let target_codec = LocalLogCheckpointJsonCodecV3::new(
        request.target_context().clone(),
        request.target_checkpoint_binding().clone(),
    )
...
#[test]
fn admission_prepares_one_unpublished_exact_v3_storage_root() -> TestResult {
    let source = source_anchor(2, true)?;
    let source_state = source.session().state().clone();
    let source_history = source.session().history_status();
    let request = target_request()?;
    ...
    let codec = LocalLogStorageRootJsonCodecV3::new(request.target_context().clone(), root_binding);
...
fn identity_and_same_fingerprint_rejections_leave_source_unchanged() -> TestResult {
    ...
    let Err(error) = same_schema.try_prepare_v3(&source) else {
    ...
    let Err(error) = reused_lineage.try_prepare_v3(&source) else {
    ...
    let Err(error) = reused_session.try_prepare_v3(&source) else {
...
fn target_rejection_and_checkpoint_policy_failure_are_non_destructive() -> TestResult {
    ...
    let before_history = source.session().history_status();
    let request = target_request()?;
    let Err(error) = request.try_prepare_v3(&source) else {
    ...
    assert!(matches!(
        error,
        SchemaAdmissionV3Error::Document(source)
    ...
    let Err(error) =
        target_request()?.with_checkpoint_limits(denied_capacity).try_prepare_v3(&source)
    else {
