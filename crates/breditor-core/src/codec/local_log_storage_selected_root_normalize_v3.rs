use super::{
    LocalLogStorageSelectedJsonCodecV3, LocalLogStorageSelectedRootError,
    LocalLogStorageSelectedRootGenerationField, LocalLogStorageSelectedRootV3,
...
impl LocalLogStorageSelectedJsonCodecV3 {
    /// Strictly normalizes one canonical selected Storage Root V3 value.
    ///
    /// The supplied trusted binding must retain the same durable schema and a
    /// Frame V3 active generation. Failure never replaces or mutates the
    /// caller's selected evidence.
    ///
    /// # Errors
    ///
    /// Returns [`LocalLogStorageSelectedRootError`] for routing, schema/frame,
    /// receipt, generation, nested-checkpoint, or strict V3 decode failure.
    pub fn normalize_root_v3(
        &self,
        json: &str,
        ...
        let selection = self.root_codec_for_v3(receipt).decode_root(json).map_err(|error| {
