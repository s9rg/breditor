//! Streaming preflight for property-preserving session history entries.
...
use super::{
    ...
    operation_sequence_v2::{
        OperationSequenceDecodeErrorV2, OperationSequencePreflightV2,
...
/// One borrowed, fully preflighted V3 history entry.
pub(crate) struct PreflightedSessionHistoryEntryV2<'a> {
    pub(crate) raw: &'a RawValue,
    pub(crate) operations: OperationSequencePreflightV2,
}

/// Failure encountered before allocating or replaying a V3 history chain.
pub(crate) enum SessionHistoryEntriesPreflightErrorV2 {
    Json(JsonFailure),
    EntryJson { entry_index: u64, source: JsonFailure },
    EmptyOperations { entry_index: u64 },
    Operations { entry_index: u64, source: OperationSequenceDecodeErrorV2 },
    AggregateOperationLimit { actual: u64, maximum: u64 },
    AggregateOperationOverflow,
}

/// Preflights the exact counted array of property-preserving history recipes.
pub(crate) fn preflight_session_history_entries_v2<'a>(
    json: &'a str,
    context: &EditorContext,
    expected_count: u64,
    maximum_operations: u64,
) -> Result<Vec<PreflightedSessionHistoryEntryV2<'a>>, SessionHistoryEntriesPreflightErrorV2> {
    let mut deferred_error = None;
    let mut deserializer = serde_json::Deserializer::from_str(json);
    let decoded = SessionHistoryEntriesSeedV2 {
        context,
        expected_count,
        maximum_operations,
    ...
    let entries = decoded.map_err(|error| {
        SessionHistoryEntriesPreflightErrorV2::Json(JsonFailure::from_serde(&error))
    })?;
...
struct SessionHistoryEntriesSeedV2<'a> {
    context: &'a EditorContext,
    expected_count: u64,
    maximum_operations: u64,
    deferred_error: &'a mut Option<SessionHistoryEntriesPreflightErrorV2>,
}

impl<'de> DeserializeSeed<'de> for SessionHistoryEntriesSeedV2<'_> {
    type Value = Vec<PreflightedSessionHistoryEntryV2<'de>>;

    fn deserialize<D>(self, deserializer: D) -> Result<Self::Value, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        deserializer.deserialize_seq(SessionHistoryEntriesVisitorV2 {
...
struct SessionHistoryEntriesVisitorV2<'a> {
    context: &'a EditorContext,
    expected_count: u64,
    maximum_operations: u64,
    deferred_error: &'a mut Option<SessionHistoryEntriesPreflightErrorV2>,
...
impl<'de> Visitor<'de> for SessionHistoryEntriesVisitorV2<'_> {
    type Value = Vec<PreflightedSessionHistoryEntryV2<'de>>;

    fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("a counted JSON array of property-preserving history entries")
    }
        ...
        A: SeqAccess<'de>,
    {
        ...
        while let Some(raw) = sequence.next_element::<&'de RawValue>()? {
            ...
            let record: BorrowedSessionHistoryEntryRecordV2<'de> =
                match serde_json::from_str(raw.get()) {
                    Ok(record) => record,
                    Err(error) => {
                        *self.deferred_error =
                            Some(SessionHistoryEntriesPreflightErrorV2::EntryJson {
                                entry_index,
            ...
            let operations = match preflight_operation_sequence_v2(
                ...
                Err(source) => {
                    *self.deferred_error =
                        Some(SessionHistoryEntriesPreflightErrorV2::Operations {
                            entry_index,
                            source,
                        });
            ...
            if operation_count == 0 {
                *self.deferred_error =
                    Some(SessionHistoryEntriesPreflightErrorV2::EmptyOperations { entry_index });
            ...
            let Some(next_aggregate) = aggregate_operations.checked_add(operation_count) else {
                *self.deferred_error =
                    Some(SessionHistoryEntriesPreflightErrorV2::AggregateOperationOverflow);
            ...
            if aggregate_operations > self.maximum_operations {
                *self.deferred_error =
                    Some(SessionHistoryEntriesPreflightErrorV2::AggregateOperationLimit {
            ...
            {
                *self.deferred_error = Some(SessionHistoryEntriesPreflightErrorV2::EntryJson {
                    entry_index,
                    source: JsonFailure::from_serde(&error),
                });
                return Err(A::Error::custom("session history result selection is invalid"));
            }
            if let Err(error) = preflight_pending_format_records_v2(
                record.result_pending_formats.get(),
                self.context,
            ) {
                *self.deferred_error = Some(SessionHistoryEntriesPreflightErrorV2::EntryJson {
                    entry_index,
            ...
            entries.push(PreflightedSessionHistoryEntryV2 { raw, operations });
...
pub(crate) struct BorrowedSessionHistoryEntryRecordV2<'a> {
    #[serde(borrow)]
