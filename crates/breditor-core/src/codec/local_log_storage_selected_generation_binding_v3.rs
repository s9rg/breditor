...
use super::{
    LocalLogStorageGenerationFrameV3, LocalLogStorageSelectedActiveGenerationBinding,
    LocalLogStorageSelectedCheckpointGenerationBinding,
    LocalLogStorageSelectedCheckpointGenerationState,
};

/// Complete Frame V3 facts for a selection envelope's checkpoint generation.
///
/// This wrapper prevents a Frame V3 policy from being projected through the
/// legacy Frame V1 accessor on the internal compatibility representation.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct LocalLogStorageSelectedCheckpointGenerationBindingV3 {
    inner: LocalLogStorageSelectedCheckpointGenerationBinding,
}

impl LocalLogStorageSelectedCheckpointGenerationBindingV3 {
    /// Creates permanent checkpoint-only facts for a selected V3 root.
    #[must_use]
    ...
    /// Creates complete retired facts for a selected V3 rotation.
    #[must_use]
    pub const fn retired(
        log_id: LocalLogId,
        session_id: LocalSessionId,
        frame: LocalLogStorageGenerationFrameV3,
        ...
        Self {
            inner: LocalLogStorageSelectedCheckpointGenerationBinding::retired_v3(
                log_id,
                session_id,
                frame,
                activated_fence_id,
    ...
    /// Creates complete reclaimed facts for a selected V3 rotation.
    #[must_use]
    pub const fn reclaimed(
        ...
        Self {
            inner: LocalLogStorageSelectedCheckpointGenerationBinding::reclaimed_v3(
                log_id,
                session_id,
                frame,
                activated_fence_id,
    ...
    /// Returns the exact Frame V3 policy, absent for checkpoint-only facts.
    #[must_use]
    pub const fn frame(&self) -> Option<LocalLogStorageGenerationFrameV3> {
        self.inner.frame_v3()
    }
...
/// Complete Frame V3 facts for a selection envelope's active generation.
///
/// The value is inspection data only and carries no mutable writer authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct LocalLogStorageSelectedActiveGenerationBindingV3 {
    inner: LocalLogStorageSelectedActiveGenerationBinding,
}

impl LocalLogStorageSelectedActiveGenerationBindingV3 {
    /// Creates one complete set of immutable active Frame V3 facts.
    #[must_use]
    pub const fn new(
        log_id: LocalLogId,
        session_id: LocalSessionId,
        ...
        Self {
            inner: LocalLogStorageSelectedActiveGenerationBinding::new_v3(
                log_id,
                session_id,
                frame,
                activated_fence_id,
    ...
    #[must_use]
    pub const fn frame(&self) -> LocalLogStorageGenerationFrameV3 {
        match self.inner.frame_v3() {
            Some(frame) => frame,
            None => unreachable!(),
        }
    }
