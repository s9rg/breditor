...
use super::{
    LocalLogStorageGenerationFrameV3, LocalLogStorageSelectedRoot, LocalLogStorageSelectionKind,
    LocalLogStorageSelectionReceiptBinding,
};

/// One trusted, normalized exact Storage V3 selection envelope.
///
/// The wrapper hides every legacy Frame V1 projection while retaining the
/// common internal normalized representation.
///
/// A V3 selected root cannot enter a V2 rotation path:
///
/// ```compile_fail
/// use breditor_core::codec::{LocalLogStorageGenerationJsonCodecV2, LocalLogStorageSelectedRootV3};
/// fn cannot_mix(codec: &LocalLogStorageGenerationJsonCodecV2, selected: &LocalLogStorageSelectedRootV3) {
///     let _ = codec.decode_rotation_from_selected("{}", selected);
/// }
/// ```
#[must_use = "a selected Storage V3 root must be explicitly handled"]
...
impl fmt::Debug for LocalLogStorageSelectedRootV3 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("LocalLogStorageSelectedRootV3")
