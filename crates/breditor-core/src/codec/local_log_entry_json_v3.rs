...
use super::{
    BoundedDiagnostic, COMMIT_FORMAT, COMMIT_V3_FORMAT_VERSION, CommitJsonCodecV3,
    CommitV3CodecError, JsonFailure, LOCAL_LOG_ENTRY_FORMAT, LocalLogCommitEventKind,
    LocalLogEntryRecordError, LocalLogEntryRecordErrorCode, LocalLogEntryRecordLocation,
    LocalLogEntryV3CodecError, json_size::JsonByteCounter,
    local_log_entry_encoding_v3::LocalLogEntryEncodingV3,
};

/// Wire version accepted and emitted by [`LocalLogEntryJsonCodecV3`].
pub const LOCAL_LOG_ENTRY_V3_FORMAT_VERSION: u32 = 3;
...
const _: () = assert!(COMMIT_V3_FORMAT_VERSION == 3);

/// Strict codec for one property-preserving ordered local-session event.
///
/// The outer binding is retained on every decoded [`LocalLogEntry`], including
/// control-only events. Commit, undo, and redo events embed Commit V3 exactly;
/// older commit generations are never accepted by this codec.
#[derive(Clone, Debug)]
pub struct LocalLogEntryJsonCodecV3 {
    context: EditorContext,
    commit_codec: CommitJsonCodecV3,
}

impl LocalLogEntryJsonCodecV3 {
    /// Creates a V3 entry codec bound to one immutable runtime context.
    #[must_use]
    ...
    /// Strictly decodes one Local Log Entry V3 value.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`LocalLogEntryV3CodecError`] for oversized or malformed input,
    /// non-V3 routing, malformed or mismatched schema binding, invalid
    /// identities or sequence, or an invalid nested Commit V3 value.
    pub fn decode(&self, json: &str) -> Result<LocalLogEntry, LocalLogEntryV3CodecError> {
        let maximum = self.context.limits().max_json_bytes();
        if json.len() > maximum {
            return Err(LocalLogEntryV3CodecError::InputTooLarge { actual: json.len(), maximum });
        }

        let header: BorrowedLocalLogEntryHeader<'_> = decode_json(json)?;
        if header.format != LOCAL_LOG_ENTRY_FORMAT {
            return Err(LocalLogEntryV3CodecError::UnsupportedFormat {
                found: BoundedDiagnostic::from(header.format.as_ref()),
                expected: LOCAL_LOG_ENTRY_FORMAT,
            });
        }
        if header.format_version != LOCAL_LOG_ENTRY_V3_FORMAT_VERSION {
            return Err(LocalLogEntryV3CodecError::UnsupportedFormatVersion {
                found: header.format_version,
                supported: LOCAL_LOG_ENTRY_V3_FORMAT_VERSION,
            });
        }

        let envelope: BorrowedLocalLogEntryRecordV3<'_> = decode_json(json)?;
    ...
    /// Encodes one checked runtime entry into deterministic compact V3 JSON.
    ///
    /// The entry's retained selector and fingerprint must both equal the codec
    /// context. Commit-bearing events are encoded only through Commit V3.
    ///
    /// # Errors
    ///
    /// Returns [`LocalLogEntryV3CodecError`] for binding/context mismatch,
    /// invalid event proof, nested Commit V3 failure, serialization failure, or
    /// output exceeding the shared byte budget.
    pub fn encode(&self, entry: &LocalLogEntry) -> Result<String, LocalLogEntryV3CodecError> {
        require_schema_binding(self.context.schema(), entry.schema_binding())?;
        entry.event().validate().map_err(LocalLogEntryV3CodecError::InvalidEventCommit)?;
        ...
        let encoding = LocalLogEntryEncodingV3 { entry, event };
        ...
        if byte_counter.exceeded() {
            return Err(LocalLogEntryV3CodecError::OutputTooLarge {
        ...
        count_result
            ...
            .map_err(LocalLogEntryV3CodecError::Encoding)?;
        serde_json::to_string(&encoding)
            .map_err(|error| JsonFailure::from_serde(&error))
            .map_err(LocalLogEntryV3CodecError::Encoding)
    }

    fn decode_event(&self, raw: &RawValue) -> Result<LocalLogEvent, LocalLogEntryV3CodecError> {
        ...
        match header.kind {
            ...
            BorrowedLocalLogEventKind::Undo => {
                ...
                LocalLogEvent::try_undo(commit)
                    .map_err(LocalLogEntryV3CodecError::InvalidEventCommit)
            }
    ...
    ) -> Result<Commit, LocalLogEntryV3CodecError> {
        validate_nested_commit_header(raw.get(), event_kind)?;
        self.commit_codec.decode(raw.get()).map_err(|source| {
            LocalLogEntryV3CodecError::InvalidCommit { event_kind, source: Box::new(source) }
        })
    }
    ...
    fn encode_commit(
        ...
        commit: &Commit,
        event_kind: LocalLogCommitEventKind,
    ) -> Result<Box<RawValue>, LocalLogEntryV3CodecError> {
        let encoded = self.commit_codec.encode(commit).map_err(|source| match source {
            CommitV3CodecError::ContextConfigurationMismatch => {
                LocalLogEntryV3CodecError::ContextConfigurationMismatch
            }
            CommitV3CodecError::OutputTooLarge { minimum, maximum } => {
                LocalLogEntryV3CodecError::OutputTooLarge { minimum, maximum }
            }
            source => {
                LocalLogEntryV3CodecError::InvalidCommit { event_kind, source: Box::new(source) }
            }
    ...
    fn encode_event_commit(
        ...
        let commit =
            event.as_commit().ok_or_else(|| LocalLogEntryV3CodecError::RuntimeInvariant {
...
struct BorrowedLocalLogEntryRecordV3<'a> {
    #[serde(rename = "format")]
    _format: IgnoredAny,
...
fn decode_json<'de, T>(json: &'de str) -> Result<T, LocalLogEntryV3CodecError>
where
    T: Deserialize<'de>,
{
    serde_json::from_str(json)
        .map_err(|error| JsonFailure::from_serde(&error))
        .map_err(LocalLogEntryV3CodecError::InvalidJson)
}

fn decode_schema_id(
    schema: &BorrowedSchemaIdRecord<'_>,
) -> Result<SchemaId, LocalLogEntryV3CodecError> {
    let name = QualifiedName::try_new(schema.name.as_ref()).map_err(|source| {
        LocalLogEntryV3CodecError::InvalidSchemaName {
            value: BoundedDiagnostic::from(schema.name.as_ref()),
            source,
        }
    })?;
    let version = SchemaVersion::try_new(schema.version).map_err(|source| {
        LocalLogEntryV3CodecError::InvalidSchemaVersion { value: schema.version, source }
    })?;
    Ok(SchemaId::new(name, version))
}

fn decode_session_id(raw: &RawValue) -> Result<LocalSessionId, LocalLogEntryV3CodecError> {
...
fn decode_log_id(raw: &RawValue) -> Result<LocalLogId, LocalLogEntryV3CodecError> {
    preflight_bounded_string(
        raw,
        MAX_IDENTITY_STRING_JSON_BYTES,
...
fn decode_sequence(raw: &RawValue) -> Result<LocalLogSequence, LocalLogEntryV3CodecError> {
    preflight_bounded_string(
        raw,
...
fn decode_replay_id(raw: &RawValue) -> Result<ReplayId, LocalLogEntryV3CodecError> {
    preflight_bounded_string(
        raw,
        MAX_IDENTITY_STRING_JSON_BYTES,
...
) -> Result<(), LocalLogEntryV3CodecError> {
...
fn validate_nested_commit_header(
    ...
    let header: BorrowedNestedCommitHeader<'_> = serde_json::from_str(json)
        ...
        .map_err(|source| LocalLogEntryV3CodecError::InvalidCommit {
            event_kind,
            source: Box::new(source),
        })?;
    if header.format != COMMIT_FORMAT {
        return Err(LocalLogEntryV3CodecError::InvalidCommit {
            event_kind,
            source: Box::new(CommitV3CodecError::UnsupportedFormat {
                found: BoundedDiagnostic::from(header.format.as_ref()),
    ...
    if header.format_version != COMMIT_V3_FORMAT_VERSION {
        return Err(LocalLogEntryV3CodecError::InvalidCommit {
            event_kind,
            source: Box::new(CommitV3CodecError::UnsupportedFormatVersion {
                found: header.format_version,
