...
/// A typed failure while decoding, encoding, or preparing Storage Root V3.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum LocalLogStorageRootV3CodecError {
    /// A retained value was proved under another runtime context.
    #[error("storage-root V3 context differs from the retained checkpoint context")]
    ContextConfigurationMismatch,
    /// The complete input exceeds the root-input limit.
    #[error("storage-root V3 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
    ...
    #[error(
        "encoded storage-root V3 JSON exceeds {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid storage-root V3 JSON: {0}")]
    ...
    /// The envelope does not use Storage Root V3.
    ...
    #[error("storage-root V3 {field:?} does not match its trusted association")]
    BindingMismatch {
    ...
    /// The decoded checkpoint string is not exact canonical Checkpoint V3 JSON.
    #[error("storage-root checkpoint JSON is not canonical Checkpoint V3 output")]
    NonCanonicalCheckpointJson,
    /// The complete input is not exact canonical Storage Root V3 JSON.
    #[error("storage-root JSON is not its canonical V3 byte encoding")]
    NonCanonicalRootJson,
    /// A checked runtime invariant failed after validation.
    #[error("storage-root V3 runtime invariant failed: {diagnostic}")]
    RuntimeInvariant {
        /// Bounded payload-free invariant diagnostic.
        diagnostic: BoundedDiagnostic,
    },
    /// Serialization of a checked value failed.
    #[error("could not encode storage-root V3 JSON: {0}")]
    Encoding(LocalLogStorageRootJsonFailure),
}

impl LocalLogStorageRootV3CodecError {
