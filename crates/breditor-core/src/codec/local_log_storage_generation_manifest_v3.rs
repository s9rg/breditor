...
use super::{LocalLogStorageGenerationFrameV3, LocalLogStorageGenerationManifest};

/// One checked, canonicalizable Storage Generation V3 rotation.
///
/// The opaque wrapper exposes only Frame V3 policies and prevents callers from
/// projecting them through legacy Frame V1 accessors.
#[must_use = "a V3 storage-generation manifest must be explicitly handled"]
#[derive(Eq, PartialEq)]
pub struct LocalLogStorageGenerationManifestV3(LocalLogStorageGenerationManifest);

impl LocalLogStorageGenerationManifestV3 {
    ...
    /// Returns the exact sealed Frame V3 policy.
    #[must_use]
    pub const fn sealed_frame(&self) -> LocalLogStorageGenerationFrameV3 {
        match self.0.sealed_frame_v3() {
            Some(frame) => frame,
            None => unreachable!(),
        }
    }

    /// Returns the exact successor Frame V3 policy.
    #[must_use]
    pub const fn successor_frame(&self) -> LocalLogStorageGenerationFrameV3 {
        match self.0.successor_frame_v3() {
            Some(frame) => frame,
            None => unreachable!(),
        }
    }

    /// Returns exact canonical embedded Checkpoint V3 JSON.
    #[must_use]
...
impl fmt::Debug for LocalLogStorageGenerationManifestV3 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("LocalLogStorageGenerationManifestV3")
