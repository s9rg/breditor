...
use super::{
    BoundedDiagnostic, CodecErrorCode, LocalLogCheckpointV3CodecError,
...
/// A typed failure while decoding, encoding, or preparing Storage Generation V3.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum LocalLogStorageGenerationV3CodecError {
    /// A retained value was proved under another runtime context.
    #[error("storage-generation V3 context differs from the retained checkpoint context")]
    ContextConfigurationMismatch,
    /// The complete input exceeds the manifest-input limit.
    #[error("storage-generation V3 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
    ...
    #[error(
        "encoded storage-generation V3 JSON exceeds {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid storage-generation V3 JSON: {0}")]
    ...
    /// The envelope does not use Storage Generation V3.
    #[error(
    ...
    #[error("storage-generation V3 {field:?} does not match its trusted association")]
    BindingMismatch {
    ...
    /// The embedded Checkpoint V3 failed strict reconstruction or encoding.
    #[error("invalid nested Local Log Checkpoint V3: {0}")]
    InvalidCheckpoint(#[source] Box<LocalLogCheckpointV3CodecError>),
    /// The checkpoint string is not exact canonical Checkpoint V3 JSON.
    #[error("storage-generation checkpoint JSON is not canonical Checkpoint V3 output")]
    NonCanonicalCheckpointJson,
    /// The complete input is not exact canonical Storage Generation V3 JSON.
    #[error("storage-generation JSON is not its canonical V3 byte encoding")]
    NonCanonicalManifestJson,
    /// A checked runtime invariant failed after validation.
    #[error("storage-generation V3 runtime invariant failed: {diagnostic}")]
    RuntimeInvariant {
        /// Bounded payload-free invariant diagnostic.
        diagnostic: BoundedDiagnostic,
    },
    /// Serialization of a checked value failed.
    #[error("could not encode storage-generation V3 JSON: {0}")]
    Encoding(LocalLogStorageGenerationJsonFailure),
}

impl LocalLogStorageGenerationV3CodecError {
