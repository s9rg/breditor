//! Deterministic property-preserving session-checkpoint encodings.
...
use super::{
    ...
    session_checkpoint_json_v3::SESSION_CHECKPOINT_V3_FORMAT_VERSION,
};

/// Borrowed deterministic encoding of one bounded session as Session Checkpoint V3.
pub(crate) struct SessionCheckpointEncodingV3<'a, Entries> {
    pub(crate) context: &'a EditorContext,
    pub(crate) history_base: EditorStateEncodingV3<'a>,
...
impl<Entries> Serialize for SessionCheckpointEncodingV3<'_, Entries>
where
    Entries: Serialize,
{
    ...
    where
        S: serde::Serializer,
    {
        let mut record = serializer.serialize_struct("SessionCheckpointRecordV3", 11)?;
        record.serialize_field("format", SESSION_CHECKPOINT_FORMAT)?;
        record.serialize_field("formatVersion", &SESSION_CHECKPOINT_V3_FORMAT_VERSION)?;
