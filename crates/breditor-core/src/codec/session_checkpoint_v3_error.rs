//! Typed failures for property-preserving session checkpoints.
...
use super::{
    BoundedDiagnostic, CodecErrorCode, EditorStateV3CodecError, JsonFailure,
    SessionCheckpointCodecError,
};

/// A typed failure while decoding or encoding one Session Checkpoint V3 value.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum SessionCheckpointV3CodecError {
    /// The supplied session was proved under a different complete runtime context.
    #[error("session-checkpoint V3 codec context differs from the supplied session context")]
    ContextConfigurationMismatch,
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("session-checkpoint V3 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded session-checkpoint V3 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid session-checkpoint V3 JSON: {0}")]
    ...
    /// The envelope uses a wire version other than Session Checkpoint V3.
    #[error(
    ...
    /// The embedded normalized history base failed its Editor State V3 boundary.
    #[error("invalid session-checkpoint V3 history base: {0}")]
    InvalidHistoryBase(#[source] Box<EditorStateV3CodecError>),
    /// Checkpoint topology, payload, resource, replay, or assembly validation failed.
    #[error("invalid session-checkpoint V3 payload: {0}")]
    InvalidCheckpoint(#[source] Box<SessionCheckpointCodecError>),
    /// Serialization of a checked session failed.
    #[error("could not encode session-checkpoint V3 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl SessionCheckpointV3CodecError {
