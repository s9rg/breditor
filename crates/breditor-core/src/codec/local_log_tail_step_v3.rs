...
use super::{LocalLogFrameTruncationV3, LocalLogTailCursorV3};

/// Non-failing result of scanning and possibly admitting one Frame V3.
#[non_exhaustive]
#[derive(Clone, Copy, Eq, PartialEq)]
pub enum LocalLogTailStatusV3 {
    /// The supplied slice is empty at the exact accepted V3 boundary.
    EndOfInput,
    /// The supplied bytes are one valid incomplete Frame V3 prefix.
    Truncated(LocalLogFrameTruncationV3),
    /// One complete Frame V3 was semantically accepted.
    Accepted {
...
impl LocalLogTailStatusV3 {
    ...
    /// Returns the admission result for one accepted complete Frame V3.
    #[must_use]
...
impl fmt::Debug for LocalLogTailStatusV3 {
...
/// Complete owned Frame V3 cursor plus one non-failing observation status.
pub struct LocalLogTailStepV3 {
    cursor: LocalLogTailCursorV3,
    status: LocalLogTailStatusV3,
}

impl LocalLogTailStepV3 {
    pub(super) const fn new(cursor: LocalLogTailCursorV3, status: LocalLogTailStatusV3) -> Self {
        Self { cursor, status }
    }

    /// Returns the unchanged or advanced V3 cursor published by this step.
    #[must_use]
    ...
    #[must_use]
    pub const fn status(&self) -> LocalLogTailStatusV3 {
        self.status
    }

    /// Separates the cursor from its observation status.
    #[must_use]
    pub fn into_parts(self) -> (LocalLogTailCursorV3, LocalLogTailStatusV3) {
        (self.cursor, self.status)
    }
}

impl fmt::Debug for LocalLogTailStepV3 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("LocalLogTailStepV3")
