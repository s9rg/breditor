//! Checked conversion between property-preserving V2 payloads and operations.
...
use crate::{
    ...
    document::{Format, FormatSet, TextFragment, TextRun},
    ...
    record::{
        OperationFormatRecordV2, OperationRecordV2, RootTextBoundaryRecordV1,
        RootTextRangeRecordV1, TextFragmentRecordV2, TextRangeRecordV1, TextRunRecordV2,
    },
};

use super::property_payload::{
    PropertyPayloadError, decode_property_map_record, encode_property_map_record,
};

/// Reconstructs one strict property-preserving payload through checked runtime
/// constructors. Context-static schema validation remains the caller's next
/// admission step.
pub(crate) fn decode_operation_payload_v2(
    record: OperationRecordV2,
) -> Result<Operation, OperationRecordError> {
    match record {
        OperationRecordV2::TextSplice { range, expected_removed, replacement } => {
        ...
        OperationRecordV2::ParagraphSplit { paragraph_path, offset, expected } => {
            let paragraph_path =
        ...
        OperationRecordV2::ParagraphJoin { left_path, expected_left, expected_right } => {
        ...
        OperationRecordV2::RootTextReplace {
            range,
            expected_paragraphs,
...
/// Projects one checked runtime operation into a property-preserving payload.
pub(crate) fn encode_operation_payload_v2(operation: &Operation) -> OperationRecordV2 {
    match operation {
        Operation::TextSplice(operation) => OperationRecordV2::TextSplice {
        ...
        Operation::ParagraphSplit(operation) => OperationRecordV2::ParagraphSplit {
        ...
        Operation::ParagraphJoin(operation) => OperationRecordV2::ParagraphJoin {
        ...
        Operation::RootTextReplace(operation) => OperationRecordV2::RootTextReplace {
...
fn fragment_list_from_record(
    records: Vec<TextFragmentRecordV2>,
...
fn format_from_record(
    record: OperationFormatRecordV2,
    field: OperationFragmentField,
    paragraph_index: u64,
    run_index: u64,
    format_index: u64,
    ...
    let properties = decode_property_map_record(record.properties).map_err(|error| {
        let code = match &error {
            PropertyPayloadError::InvalidPropertyName { .. } => {
                OperationRecordErrorCode::InvalidPropertyName
            }
            PropertyPayloadError::InvalidInteger { .. }
            | PropertyPayloadError::InvalidObject { .. }
            | PropertyPayloadError::NonCanonicalMap { .. } => {
                OperationRecordErrorCode::InvalidPropertyValue
            }
        };
...
fn fragment_record_from_runtime(fragment: &TextFragment) -> TextFragmentRecordV2 {
    TextFragmentRecordV2 {
        runs: fragment
            .iter()
            .map(|run| TextRunRecordV2 {
                text: run.text().to_owned(),
                formats: run
                    .formats()
                    .iter()
                    .map(|format| OperationFormatRecordV2 {
