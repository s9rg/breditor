...
#[path = "local_log_storage_typed_v3_tests.rs"]
mod typed;
...
use super::{
    ...
    LOCAL_LOG_STORAGE_ROOT_FORMAT, LocalLogCheckpointJsonCodecV3, LocalLogFrameLimits,
    LocalLogStorageGenerationBinding, LocalLogStorageGenerationJsonCodecV3,
    LocalLogStorageGenerationLimits, LocalLogStorageGenerationPreparationInputs,
    LocalLogStorageGenerationV3CodecError, LocalLogStorageRootBinding,
    LocalLogStorageRootCodecError, LocalLogStorageRootJsonCodec, LocalLogStorageRootJsonCodecV3,
    LocalLogStorageRootPreparationInputs, LocalLogStorageRootResourceLimit,
    LocalLogStorageRootV3CodecError, LocalLogStorageSelectedActiveGenerationBindingV3,
    LocalLogStorageSelectedBindingV3, LocalLogStorageSelectedCheckpointGenerationBindingV3,
    LocalLogStorageSelectedJsonCodecV3, LocalLogStorageSelectedRootV3,
...
const PROFILE: &str = "breditor/storage-v3-tests";
const SCOPE: &str = "scope:storage-v3-tests";
const DATABASE_INCARNATION: &str = "database:storage-v3-tests";
const SCOPE_INCARNATION: &str = "scope-incarnation:storage-v3-tests";
const SESSION: &str = "session:storage-v3-tests";
const ROOT_TRANSACTION: &str = "transaction:storage-v3-tests:root";
const ROOT_HEAD: &str = "head:storage-v3-tests:h0";
const ROOT_FENCE: &str = "fence:storage-v3-tests:f0";
const ROTATION_TRANSACTION: &str = "transaction:storage-v3-tests:rotation";
const ROTATION_HEAD: &str = "head:storage-v3-tests:h1";
const ROTATION_FENCE: &str = "fence:storage-v3-tests:f1";

struct StorageV3Fixture {
    context: EditorContext,
    root_binding: LocalLogStorageRootBinding,
    root_outcome: super::LocalLogTailCompactionOutcomeV3,
...
impl StorageV3Fixture {
    fn new() -> TestResult<Self> {
        let context = EditorContext::default();
        let document = DocumentJsonCodec::new(context.schema().clone())
            .with_limits(context.limits().clone())
            .decode(
                r#"{"format":"breditor/document","formatVersion":1,"schema":{"name":"breditor/base","version":1},"root":{"kind":"element","type":"breditor/document","entityId":null,"properties":{},"children":[{"kind":"element","type":"breditor/paragraph","entityId":null,"properties":{},"children":[{"kind":"text","text":"STORAGEV3PAYLOADSENTINEL","formats":[]}]}]}}"#,
            )?;
        let state = EditorState::try_new(
            &context,
            LineageId::try_new("storage-v3-tests")?,
            document,
            None,
            None,
        )?;
        Self::with_session(EditorSession::new(state))
    }

    fn with_session(session: EditorSession) -> TestResult<Self> {
        Self::with_events(session, Vec::new())
    }

    fn with_events(
        session: EditorSession,
        events: Vec<crate::local_log::LocalLogEntry>,
    ) -> TestResult<Self> {
        ...
        .recover(session, events)?
        .try_into_checkpoint_anchor(
            LocalLogId::try_new("log:storage-v3-tests:g1")?,
            LocalLogCompactionLimits::default(),
        )?;
        let sealed_frame_limits = LocalLogFrameLimits::new(4_096);
        let cursor =
            initial.begin_successor_tail_v3(LocalLogRecoveryLimits::default(), sealed_frame_limits);
        let (owner, _, retained_limits, _) = cursor.into_parts();
        let root_outcome = LocalLogTailCursorV3::from_trusted_parts(owner, 123, retained_limits)
            .try_into_checkpoint_anchor(LocalLogId::try_new("log:storage-v3-tests:g2")?)?;
    ...
    fn root_codec(&self) -> LocalLogStorageRootJsonCodecV3 {
        LocalLogStorageRootJsonCodecV3::new(self.context.clone(), self.root_binding.clone())
    }
    ...
    fn normalize_root(&self, json: &str) -> TestResult<LocalLogStorageSelectedRootV3> {
        let root = self.root_codec().decode_root(json)?;
        let binding = LocalLogStorageSelectedBindingV3::try_new(
        ...
        LocalLogStorageSelectedJsonCodecV3::new(self.context.clone(), binding)
            .normalize_root_v3(json)
            .map_err(Into::into)
    }
    ...
    fn rotation_outcome(
    ...
    ) -> TestResult<super::LocalLogTailCompactionOutcomeV3> {
        let checkpoint = LocalLogCheckpointJsonCodecV3::new(
            self.context.clone(),
        ...
        let cursor = checkpoint.begin_successor_tail_v3(
            LocalLogRecoveryLimits::default(),
            selected.active_frame().limits(),
        );
        let (owner, _, retained_limits, _) = cursor.into_parts();
        LocalLogTailCursorV3::from_trusted_parts(owner, 57, retained_limits)
            .try_into_checkpoint_anchor(LocalLogId::try_new("log:storage-v3-tests:g3")?)
            .map_err(Into::into)
    }

    fn rotation_codec(&self) -> TestResult<LocalLogStorageGenerationJsonCodecV3> {
        Ok(LocalLogStorageGenerationJsonCodecV3::new(
            self.context.clone(),
    ...
    fn first_rotation(
        ...
        super::LocalLogStorageGenerationManifestV3,
        String,
    )> {
        let selected = self.normalize_root(&self.encoded_root()?)?;
        let outcome = self.rotation_outcome(&selected)?;
        ...
        Ok((selected, manifest, json))
    }
}

#[test]
fn root_v3_prepare_encode_decode_and_normalize_are_exact() -> TestResult {
    ...
    let prefix = format!(
        r#"{{"format":"{LOCAL_LOG_STORAGE_ROOT_FORMAT}","formatVersion":3,"schema":{{"name":"breditor/base","version":1}},"schemaFingerprint":"{}""#,
        fixture.context.schema().fingerprint()
    );
    assert!(encoded.starts_with(&prefix));
    assert!(
        selection.checkpoint_json().starts_with(&format!(
            r#"{{"format":"{LOCAL_LOG_CHECKPOINT_FORMAT}","formatVersion":3"#
        ))
    );
    assert_eq!(selection.schema_binding(), &fixture.context.schema().durable_binding());
    assert_eq!(selection.active_frame().format_version(), 3);
...
#[test]
fn root_v3_routing_fingerprint_frame_and_limits_fail_closed() -> TestResult {
    let fixture = StorageV3Fixture::new()?;
    let encoded = fixture.encoded_root()?;
    assert!(matches!(
        LocalLogStorageRootJsonCodec::new(fixture.context.clone(), fixture.root_binding.clone())
            .decode_root(&encoded),
        Err(LocalLogStorageRootCodecError::UnsupportedFormatVersion { found: 3, supported: 1 })
    ));
    ...
    let v1_outcome = LocalLogTailCursor::from_trusted_parts(owner, 0, limits)
        .try_into_checkpoint_anchor(LocalLogId::try_new("log:storage-v3-tests:v1-active")?)?;
    let v1_codec =
        LocalLogStorageRootJsonCodec::new(fixture.context.clone(), fixture.root_binding.clone());
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::UnsupportedFormatVersion { found: 1, supported: 3 })
    ));
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::InvalidSchemaFingerprint(_))
    ));
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::SchemaBinding(_))
    ));
    let mixed_frame = encoded.replacen(
        r#""activeFrame":{"formatVersion":3"#,
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::InvalidRecord(_))
    ));
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::InputTooLarge { .. })
    ));
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::ResourceLimit(
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageRootV3CodecError::OutputTooLarge { maximum: 0, .. })
    ));
    Ok(())
}

#[test]
fn selected_root_rotates_and_normalizes_without_losing_v3_binding() -> TestResult {
    let fixture = StorageV3Fixture::new()?;
    ...
    assert!(encoded.starts_with(&format!(
        r#"{{"format":"{LOCAL_LOG_STORAGE_GENERATION_FORMAT}","formatVersion":3,"schema":{{"name":"breditor/base","version":1}},"schemaFingerprint":"{}""#,
        fixture.context.schema().fingerprint()
    )));
    ...
    assert_eq!(manifest.successor_frame().format_version(), 3);
    ...
    let selected_binding = LocalLogStorageSelectedBindingV3::try_new(
        current_receipt,
        Some(predecessor_receipt),
        LocalLogStorageSelectedCheckpointGenerationBindingV3::retired(
    ...
    let normalized =
        LocalLogStorageSelectedJsonCodecV3::new(fixture.context.clone(), selected_binding)
            .normalize_rotation_v3(&encoded, &root_json)?;
    ...
    let drifted_codec = LocalLogStorageGenerationJsonCodecV3::new(
        variant_context,
        LocalLogStorageGenerationBinding::try_new(
    ...
    assert!(matches!(
        ...
        Err(LocalLogStorageGenerationV3CodecError::ContextConfigurationMismatch)
    ));
    Ok(())
}
