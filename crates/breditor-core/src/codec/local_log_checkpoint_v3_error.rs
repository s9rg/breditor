//! Typed failures for Local Log Checkpoint V3.
...
/// A typed failure while decoding or encoding one Local Log Checkpoint V3 value.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum LocalLogCheckpointV3CodecError {
    /// The retained session was proved under a different complete runtime context.
    #[error("local-log-checkpoint V3 codec context differs from its retained session context")]
    ContextConfigurationMismatch,
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("local-log-checkpoint V3 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded local-log-checkpoint V3 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid local-log-checkpoint V3 JSON: {0}")]
    ...
    /// The envelope uses a wire version other than Local Log Checkpoint V3.
    #[error(
    ...
    #[error("invalid local-log-checkpoint V3 payload: {0}")]
    InvalidCheckpoint(#[source] Box<LocalLogCheckpointCodecError>),
    /// The embedded complete Session Checkpoint V3 failed its codec boundary.
    #[error("invalid nested Session Checkpoint V3: {0}")]
    InvalidSessionCheckpoint(#[source] Box<SessionCheckpointV3CodecError>),
    /// Serialization of a checked V3 checkpoint failed.
    #[error("could not encode local-log-checkpoint V3 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl LocalLogCheckpointV3CodecError {
