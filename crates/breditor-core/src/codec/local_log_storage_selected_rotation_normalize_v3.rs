...
use super::{
    LocalLogStorageGenerationFrameV3, LocalLogStorageSelectedJsonCodecV3,
    LocalLogStorageSelectedRootError, LocalLogStorageSelectedRootGenerationField,
    LocalLogStorageSelectedRootV3, LocalLogStorageSelectedRootValueRole,
...
struct DecodedPredecessorActiveV3 {
...
impl LocalLogStorageSelectedJsonCodecV3 {
    /// Strictly normalizes a selected Storage Generation V3 and its V3 predecessor.
    ///
    /// Both receipts must retain the receiving context's durable schema binding;
    /// both exact values must use Frame V3 and embed Checkpoint V3. Failure
    ...
    /// receipt, generation, checkpoint, or strict V3 decode failure.
    #[allow(clippy::too_many_lines)]
    pub fn normalize_rotation_v3(
        &self,
        current_json: &str,
        predecessor_json: &str,
    ) -> Result<LocalLogStorageSelectedRootV3, LocalLogStorageSelectedRootError> {
        self.require_current_kind(LocalLogStorageSelectionKind::Rotation)?;
        let current_receipt = self.binding().current_receipt();
        let current = self
            .generation_codec_for_v3(current_receipt)?
        ...
        let predecessor =
            self.decode_predecessor_active_v3(predecessor_receipt, predecessor_json)?;
        ...
        let checkpoint_anchor = self.decode_checkpoint_anchor_v3(
        ...
        .map(LocalLogStorageSelectedRootV3::new)
    }

    fn decode_predecessor_active_v3(
        &self,
        receipt: &LocalLogStorageSelectionReceiptBinding,
        json: &str,
    ) -> Result<DecodedPredecessorActiveV3, LocalLogStorageSelectedRootError> {
        let role = LocalLogStorageSelectedRootValueRole::Predecessor;
        match receipt.selection_kind() {
            LocalLogStorageSelectionKind::Root => {
                let value = self.root_codec_for_v3(receipt).decode_root(json).map_err(|error| {
            ...
            LocalLogStorageSelectionKind::Rotation => {
                let value = self
                    .generation_codec_for_v3(receipt)?
                ...
                Ok(DecodedPredecessorActiveV3 {
                    ...
                    rotation_sealed_generation: Some(
                        LocalLogStoragePredecessorRotationSealedGeneration::new_v3(
