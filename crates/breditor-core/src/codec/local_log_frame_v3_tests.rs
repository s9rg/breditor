...
use crate::{
    codec::{
        ...
        LOCAL_LOG_FRAME_V2_FORMAT_VERSION, LOCAL_LOG_FRAME_V3_FORMAT_VERSION,
        LocalLogEntryJsonCodec, LocalLogEntryJsonCodecV2, LocalLogEntryJsonCodecV3,
        LocalLogEntryV2CodecError, LocalLogEntryV3CodecError, LocalLogFrameBinding,
        LocalLogFrameCodec, LocalLogFrameCodecError, LocalLogFrameCodecV2, LocalLogFrameCodecV3,
        LocalLogFrameLimits, LocalLogFrameTruncationStage, LocalLogFrameV2CodecError,
        LocalLogFrameV3CodecError,
    },
    local_log::{
...
fn ids(label: &str) -> Result<(LocalSessionId, LocalLogId), Box<dyn Error>> {
    Ok((
        LocalSessionId::try_new(format!("session:frame-v3:{label}"))?,
        LocalLogId::try_new(format!("log:frame-v3:{label}:g1"))?,
    ))
}
...
#[test]
fn minimal_control_frame_has_exact_v3_header_and_fingerprint_bound_payload() -> TestResult {
    assert_eq!(LOCAL_LOG_FRAME_FORMAT_VERSION, 1);
    assert_eq!(LOCAL_LOG_FRAME_V2_FORMAT_VERSION, 2);
    assert_eq!(LOCAL_LOG_FRAME_V3_FORMAT_VERSION, 3);
    assert_eq!(LOCAL_LOG_FRAME_HEADER_BYTES, 28);

    let context = EditorContext::default();
    let (session, log) = ids("golden")?;
    let entry = control_entry(context.schema(), &session, &log, 1, "request:frame-v3-golden")?;
    let expected_payload = r#"{"format":"breditor/local-log-entry","formatVersion":3,"schema":{"name":"breditor/base","version":1},"schemaFingerprint":"sha256:68aecbceb27b88171cf2f64f4ff6af8f4372fb338467eafd5fbf89ab04401173","sessionId":"session:frame-v3:golden","logId":"log:frame-v3:golden:g1","sequence":"1","replayId":"request:frame-v3-golden","event":{"kind":"clearHistory"}}"#;
    assert_eq!(LocalLogEntryJsonCodecV3::new(context.clone()).encode(&entry)?, expected_payload);

    let codec = LocalLogFrameCodecV3::new(context, binding(&session, &log));
    assert_eq!(codec.format_version(), 3);
    assert_eq!(codec.schema_binding(), entry.schema_binding());
    let frame = codec.encode(&entry)?;
    let expected_header = [
        0x89, 0x42, 0x52, 0x44, 0x54, 0x4c, 0x0d, 0x0a, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x01, 0x5e, 0xc3, 0x30, 0x41, 0x9f, 0x55, 0xc3, 0xe5, 0x74,
    ];
    ...
    let borrowed = codec.scan(&frame)?.into_frame().ok_or("encoded V3 frame was incomplete")?;
...
#[test]
fn v1_v2_and_v3_frames_are_mutually_rejected_after_checksum_validation() -> TestResult {
    let context = EditorContext::default();
    ...
    let v3 = LocalLogFrameCodecV3::new(context.clone(), trusted.clone()).encode(&entry)?;

    assert!(matches!(
        LocalLogFrameCodec::new(context.clone(), trusted.clone()).scan(&v3),
        Err(LocalLogFrameCodecError::UnsupportedVersion { found: 3, supported: 1 })
    ));
    assert!(matches!(
        LocalLogFrameCodecV2::new(context.clone(), trusted.clone()).scan(&v3),
        Err(LocalLogFrameV2CodecError::UnsupportedVersion { found: 3, supported: 2 })
    ));
    assert!(matches!(
        LocalLogFrameCodecV3::new(context.clone(), trusted.clone()).scan(&v1),
        Err(LocalLogFrameV3CodecError::UnsupportedVersion { found: 1, supported: 3 })
    ));
    assert!(matches!(
        LocalLogFrameCodecV3::new(context, trusted).scan(&v2),
        Err(LocalLogFrameV3CodecError::UnsupportedVersion { found: 2, supported: 3 })
    ));
    Ok(())
}

#[test]
fn v1_v2_and_v3_payloads_cannot_be_rewrapped_across_frame_generations() -> TestResult {
    let context = EditorContext::default();
    ...
    let v2_codec = LocalLogFrameCodecV2::new(context.clone(), trusted.clone());
    let v3_codec = LocalLogFrameCodecV3::new(context.clone(), trusted);
    let v1_json = LocalLogEntryJsonCodec::new(context.clone()).encode(&entry)?;
    let v2_json = LocalLogEntryJsonCodecV2::new(context.clone()).encode(&entry)?;
    let v3_json = LocalLogEntryJsonCodecV3::new(context).encode(&entry)?;

    let v1_payload_in_v3 = raw_frame(v1_json.as_bytes(), 3)?;
    let borrowed = v3_codec
        .scan(&v1_payload_in_v3)?
        .into_frame()
        .ok_or("V1 payload under V3 header was incomplete")?;
    assert!(matches!(
        v3_codec.decode_frame(borrowed),
        Err(LocalLogFrameV3CodecError::InvalidEntry(source))
            if matches!(*source, LocalLogEntryV3CodecError::UnsupportedFormatVersion {
                found: 1,
                supported: 3,
            })
    ));
    let v2_payload_in_v3 = raw_frame(v2_json.as_bytes(), 3)?;
    let borrowed = v3_codec
        .scan(&v2_payload_in_v3)?
        .into_frame()
        .ok_or("V2 payload under V3 header was incomplete")?;
    assert!(matches!(
        v3_codec.decode_frame(borrowed),
        Err(LocalLogFrameV3CodecError::InvalidEntry(source))
            if matches!(*source, LocalLogEntryV3CodecError::UnsupportedFormatVersion {
    ...
    let v3_payload_in_v1 = raw_frame(v3_json.as_bytes(), 1)?;
    let borrowed = v1_codec
        .scan(&v3_payload_in_v1)?
        .into_frame()
        .ok_or("V3 payload under V1 header was incomplete")?;
    assert!(matches!(
        v1_codec.decode_frame(borrowed),
        Err(LocalLogFrameCodecError::InvalidEntry(_))
    ));
    let v3_payload_in_v2 = raw_frame(v3_json.as_bytes(), 2)?;
    let borrowed = v2_codec
        .scan(&v3_payload_in_v2)?
        .into_frame()
        .ok_or("V3 payload under V2 header was incomplete")?;
    assert!(matches!(
...
#[test]
fn v3_scanner_distinguishes_every_truncation_phase_and_borrows_exact_slices() -> TestResult {
    let context = EditorContext::default();
    ...
    for cut in 1..frame.len() {
        let truncation = codec
            ...
            .truncation()
            .copied()
            .ok_or("proper V3 prefix was not reported as truncated")?;
    ...
    let borrowed =
        codec.scan(&concatenated)?.into_frame().ok_or("complete V3 frame was not returned")?;
    assert_eq!(borrowed.consumed_bytes(), frame.len());
...
#[test]
fn v3_checksums_utf8_and_payload_limits_fail_at_their_owned_boundaries() -> TestResult {
    let context = EditorContext::default();
    let (session, log) = ids("boundaries")?;
    let entry = control_entry(context.schema(), &session, &log, 1, "request:boundaries")?;
    let codec = LocalLogFrameCodecV3::new(context.clone(), binding(&session, &log));
    let frame = codec.encode(&entry)?;
    ...
    assert!(matches!(
        ...
        Err(LocalLogFrameV3CodecError::HeaderChecksumMismatch { .. })
    ));
    ...
    *bad_payload.last_mut().ok_or("encoded frame was empty")? ^= 1;
    assert!(matches!(
        ...
        Err(LocalLogFrameV3CodecError::PayloadChecksumMismatch { .. })
    ));

    let payload_bytes = u64::try_from(frame.len() - LOCAL_LOG_FRAME_HEADER_BYTES)?;
    let exact = LocalLogFrameCodecV3::new(context.clone(), binding(&session, &log))
        .with_limits(LocalLogFrameLimits::new(payload_bytes));
    assert!(exact.scan(&frame)?.frame().is_some());
    assert_eq!(exact.encode(&entry)?, frame);
    let limited = LocalLogFrameCodecV3::new(context, binding(&session, &log))
        .with_limits(LocalLogFrameLimits::new(payload_bytes - 1));
    assert!(matches!(
        limited.scan(&frame),
        Err(LocalLogFrameV3CodecError::PayloadTooLarge { actual, maximum })
    ...
    let invalid_utf8 = raw_frame(&[0xff], 3)?;
    let borrowed = codec
        .scan(&invalid_utf8)?
        .into_frame()
    ...
    assert!(matches!(
        ...
        Err(LocalLogFrameV3CodecError::InvalidUtf8 { valid_up_to: 0 })
    ));
    Ok(())
}
...
fn schema_bound_control_frames_round_trip_only_under_the_exact_fingerprint() -> TestResult {
    ...
    let variant = LocalLogFrameCodecV3::new(variant_context, binding(&session, &log));
    let frame = variant.encode(&entry)?;
    let decoded = variant.decode_frame(
        variant.scan(&frame)?.into_frame().ok_or("variant V3 frame was incomplete")?,
    )?;
    assert_eq!(decoded.schema_binding(), entry.schema_binding());

    let original = frame.clone();
    let base = LocalLogFrameCodecV3::new(EditorContext::default(), binding(&session, &log));
    let borrowed = base.scan(&frame)?.into_frame().ok_or("variant V3 frame was incomplete")?;
    assert!(matches!(
        base.decode_frame(borrowed),
        Err(LocalLogFrameV3CodecError::InvalidEntry(source))
            if matches!(*source, LocalLogEntryV3CodecError::SchemaBinding(
