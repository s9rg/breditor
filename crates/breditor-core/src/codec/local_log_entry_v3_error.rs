...
use super::{
    BoundedDiagnostic, CodecErrorCode, CommitV3CodecError, JsonFailure, LocalLogCommitEventKind,
    LocalLogEntryRecordError,
};

/// A typed failure while decoding or encoding one Local Log Entry V3 value.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum LocalLogEntryV3CodecError {
    /// A nested event commit was proved under another complete runtime context.
    #[error("local-log-entry V3 codec context differs from its event commit context")]
    ContextConfigurationMismatch,
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("local-log-entry V3 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded local-log-entry V3 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    /// The outer JSON syntax or strict V3 record/event shape is invalid.
    #[error("invalid local-log-entry V3 JSON: {0}")]
    ...
    /// The envelope uses a wire version other than Local Log Entry V3.
    ...
    /// A commit-bearing event failed the authoritative Commit V3 boundary.
    #[error("invalid {event_kind} event Commit V3 value: {source}")]
    InvalidCommit {
        /// Event kind that owned the nested commit.
        event_kind: LocalLogCommitEventKind,
        /// Complete nested Commit V3 codec failure.
        #[source]
        source: Box<CommitV3CodecError>,
    },
    /// A proved nested commit violates its tagged undo or redo event contract.
    #[error("invalid local-log V3 replay event commit: {0}")]
    InvalidEventCommit(#[source] LocalLogEventError),
    /// A private runtime event disagrees with its invariant-bearing discriminator.
    #[error("local-log-entry V3 runtime invariant failed: {diagnostic}")]
    RuntimeInvariant {
        /// Bounded diagnostic that retains no commit or editor state.
        diagnostic: BoundedDiagnostic,
    },
    /// Serialization of a checked V3 entry failed.
    #[error("could not encode local-log-entry V3 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl LocalLogEntryV3CodecError {
