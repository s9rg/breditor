...
/// A typed failure while decoding or encoding one V2 transaction request.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum TransactionV2CodecError {
    /// The supplied base was proved under a different runtime context.
    #[error("transaction V2 codec context differs from the supplied base-state context")]
    ContextConfigurationMismatch,
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("transaction V2 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded transaction V2 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid transaction V2 JSON: {0}")]
    InvalidJson(#[source] JsonFailure),
    /// One primitive operation payload has invalid strict JSON shape.
    #[error("invalid transaction V2 operation JSON at index {operation_index}: {source}")]
    ...
    /// The envelope does not use the V2 transaction-request wire version.
    ...
    #[error("invalid operation at transaction V2 index {operation_index}: {source}")]
    InvalidOperation {
        /// Zero-based operation index.
        operation_index: u64,
    ...
    #[error("operation at transaction V2 index {operation_index} failed validation: {source}")]
    OperationValidation {
        /// Zero-based operation index.
        operation_index: u64,
    ...
    #[error("could not encode transaction V2 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl TransactionV2CodecError {
