...
use super::{
    LOCAL_LOG_STORAGE_GENERATION_FORMAT, LOCAL_LOG_STORAGE_GENERATION_V3_FORMAT_VERSION,
    LocalLogStorageGenerationManifest, schema_binding_encoding::SchemaBindingEncoding,
};

/// Borrowed canonical Storage Generation V3 encoding.
pub(crate) struct LocalLogStorageGenerationEncodingV3<'a> {
    value: &'a LocalLogStorageGenerationManifest,
}

impl<'a> LocalLogStorageGenerationEncodingV3<'a> {
    pub(crate) const fn new(value: &'a LocalLogStorageGenerationManifest) -> Self {
        Self { value }
    }
}

impl Serialize for LocalLogStorageGenerationEncodingV3<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let value = self.value;
        let mut record = serializer.serialize_struct("LocalLogStorageGenerationRecordV3", 18)?;
        record.serialize_field("format", LOCAL_LOG_STORAGE_GENERATION_FORMAT)?;
        record.serialize_field("formatVersion", &LOCAL_LOG_STORAGE_GENERATION_V3_FORMAT_VERSION)?;
        ...
        let sealed = value
            .sealed_frame_v3()
            .ok_or_else(|| serde::ser::Error::custom("sealed generation is not Frame V3"))?;
        let successor = value
            .successor_frame_v3()
            .ok_or_else(|| serde::ser::Error::custom("successor generation is not Frame V3"))?;
        record
            .serialize_field("sealedFrame", &FramePolicyV3(sealed.limits().max_payload_bytes()))?;
        record.serialize_field(
            "successorFrame",
            &FramePolicyV3(successor.limits().max_payload_bytes()),
        )?;
