...
/// Recoverable rejection of one consuming acquisition terminal attestation.
///
/// Both the complete unchanged owner and the unapplied attestation are
/// retained. Diagnostics deliberately omit them so document JSON and
/// misleading evidence-like values cannot leak through error formatting.
pub struct LocalLogStorageWriterFenceAcquisitionTerminalFailure<T> {
    retained: Box<(T, LocalLogStorageWriterFenceAcquisitionTerminalAttestation)>,
    error: LocalLogStorageWriterFenceAcquisitionTransitionError,
}

impl<T> LocalLogStorageWriterFenceAcquisitionTerminalFailure<T> {
    pub(super) fn new(
        owner: T,
    ...
    #[must_use = "the unapplied acquisition attestation remains available for inspection"]
    pub const fn attestation(&self) -> &LocalLogStorageWriterFenceAcquisitionTerminalAttestation {
        &self.retained.1
    }

    /// Returns the typed transition error.
    #[must_use]
    pub const fn error(&self) -> &LocalLogStorageWriterFenceAcquisitionTransitionError {
        &self.error
    }
...
impl<T> fmt::Debug for LocalLogStorageWriterFenceAcquisitionTerminalFailure<T> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("LocalLogStorageWriterFenceAcquisitionTerminalFailure")
            .field("error", &self.error)
            .finish_non_exhaustive()
    }
}

impl<T> fmt::Display for LocalLogStorageWriterFenceAcquisitionTerminalFailure<T> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.error.fmt(formatter)
    }
}

impl<T> Error for LocalLogStorageWriterFenceAcquisitionTerminalFailure<T> {
