...
/// A typed failure while decoding or encoding one V2 operation envelope.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum OperationV2CodecError {
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("operation V2 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded operation V2 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid operation V2 JSON: {0}")]
    ...
    /// The envelope does not use the V2 operation wire version.
    ...
    #[error("could not encode operation V2 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl OperationV2CodecError {
