...
use super::{
    BoundedDiagnostic, CodecErrorCode, CommitCodecError, EditorStateV2CodecError, JsonFailure,
};

/// A typed failure while decoding or encoding one durable Commit V2 value.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum CommitV2CodecError {
    /// The supplied commit was proved under a different complete runtime context.
    #[error("commit V2 codec context differs from the supplied commit context")]
    ContextConfigurationMismatch,
    /// The input exceeds the configured UTF-8 byte limit.
    #[error("commit V2 JSON is {actual} bytes; the configured maximum is {maximum}")]
    InputTooLarge {
        /// Actual input size.
        actual: usize,
    ...
    #[error(
        "encoded commit V2 JSON exceeds the configured maximum {maximum}; at least {minimum} bytes were observed"
    )]
    OutputTooLarge {
    ...
    #[error("invalid commit V2 JSON: {0}")]
    ...
    /// The envelope uses a wire version other than Commit V2.
    ...
    /// The embedded before-state checkpoint failed its Editor State V2 boundary.
    #[error("invalid commit V2 before-state checkpoint: {0}")]
    InvalidBeforeState(#[source] Box<EditorStateV2CodecError>),
    /// A commit payload, operation, or replay proof failed reconstruction.
    #[error("invalid commit V2 payload: {0}")]
    InvalidCommit(#[source] CommitCodecError),
    /// Serialization of a checked commit failed.
    #[error("could not encode commit V2 JSON: {0}")]
    Encoding(#[source] JsonFailure),
}

impl CommitV2CodecError {
