...
use crate::{
    codec::{
        LOCAL_LOG_FRAME_HEADER_BYTES, LOCAL_LOG_FRAME_V3_FORMAT_VERSION,
        LocalLogCheckpointJsonCodecV3, LocalLogFrameBinding, LocalLogFrameCodecV3,
        LocalLogFrameLimits, LocalLogTailCursorV3, LocalLogTailErrorCode, LocalLogTailErrorV3,
    },
    document::{
    ...
    local_log::{
        ...
        LocalLogId, LocalLogObservationOutcome, LocalLogRecovery, LocalLogRecoveryLimits,
        LocalLogSequence, LocalSessionId, ReplayId,
    },
    operation::{TextRange, TextSplice},
    position::{NodePath, TextOffset},
    schema::{
        CompiledSchema, DocumentLimits, DurableSchemaBinding, PersistedTypeRevision, SchemaId,
        SchemaVersion,
    },
...
fn typed_formats(href: &str) -> Result<FormatSet, Box<dyn Error>> {
    let properties =
        PropertyMap::try_from_sorted(vec![(name(TYPED_HREF)?, PropertyValue::from_string(href))])?;
...
fn fresh_tail(
    ...
    let session = LocalSessionId::try_new(format!("session:tail-v3:{label}"))?;
    let checkpoint = LocalLogId::try_new(format!("log:tail-v3:{label}:g0"))?;
    let active_log = LocalLogId::try_new(format!("log:tail-v3:{label}:g1"))?;
    let initial = state(&context, &format!("tail-v3-{label}"))?;
    ...
    let cursor = anchor.begin_successor_tail_v3(LocalLogRecoveryLimits::new(8, 8, 8), frame_limits);
...
fn assert_v3_binding(
    cursor: &LocalLogTailCursorV3,
    binding: &DurableSchemaBinding,
    offset: u64,
    observation_count: u64,
) {
    assert_eq!(cursor.frame_format_version(), LOCAL_LOG_FRAME_V3_FORMAT_VERSION);
...
-> TestResult {
    ...
    assert_v3_binding(&cursor, &expected_binding, 0, 0);

    let end = cursor.try_observe_frame(0, &[])?;
    assert!(end.status().is_end_of_input());
    assert_v3_binding(end.cursor(), &expected_binding, 0, 0);
    ...
    let entry = control_entry(
        ...
        &active_log,
        1,
        "request:tail-v3:control",
    )?;
    let codec = LocalLogFrameCodecV3::new(
        context,
    ...
    assert_v3_binding(truncated.cursor(), &expected_binding, 0, 0);
    ...
    assert_v3_binding(control_failure.cursor(), &expected_binding, 0, 0);
    let (cursor, _, _) = control_failure.into_parts();

    let successor = LocalLogId::try_new("log:tail-v3:propagation:g2")?;
    let outcome = cursor.try_into_checkpoint_anchor(successor.clone())?;
    assert_eq!(outcome.frame_format_version(), 3);
    ...
    let successor_cursor =
        anchor.begin_successor_tail_v3(LocalLogRecoveryLimits::new(8, 8, 8), retained_limits);
    assert_v3_binding(&successor_cursor, &retained_binding, 0, 0);
    assert_eq!(successor_cursor.owner().active_log_id(), &successor);

    let next_successor = LocalLogId::try_new("log:tail-v3:propagation:g3")?;
    let reauthorized = successor_cursor.try_into_checkpoint_anchor_with_compaction_limits(
        next_successor,
        LocalLogCompactionLimits::new(7),
    )?;
    assert_eq!(reauthorized.frame_format_version(), 3);
...
#[test]
fn successful_commit_and_control_transitions_keep_the_v3_binding() -> TestResult {
    ...
    let commit_entry = LocalLogEntry::new_with_schema_binding(
        ...
        ReplayId::try_new("request:tail-v3:commit")?,
    ...
    assert_v3_binding(commit_step.cursor(), &binding, commit_end, 1);
    ...
    let control = control_entry(
        ...
        &active_log,
        2,
        "request:tail-v3:control-success",
    )?;
    ...
    assert_v3_binding(control_step.cursor(), &binding, control_end, 2);
    assert_eq!(control_step.cursor().owner().session().undo_depth(), 0);
    Ok(())
}

#[test]
fn every_v3_failure_returns_the_unchanged_bound_owner_and_input() -> TestResult {
    let FreshTail { cursor, context, session, active_log } =
    ...
    assert_v3_binding(origin_failure.cursor(), &expected_binding, 0, 0);
    ...
    let wrong_entry = control_entry(
        ...
        &active_log,
        1,
        "request:tail-v3:wrong-schema",
    )?;
    let wrong_codec = LocalLogFrameCodecV3::new(
    ...
    assert_v3_binding(schema_failure.cursor(), &expected_binding, 0, 0);
    assert_eq!(wrong_frame, original);
    let (cursor, _, error) = schema_failure.into_parts();
    assert!(matches!(error, LocalLogTailErrorV3::FrameDecode(_)));
    ...
    let out_of_order = control_entry(
        ...
        &active_log,
        2,
        "request:tail-v3:out-of-order",
    )?;
    let frame =
        LocalLogFrameCodecV3::new(context, LocalLogFrameBinding::new(session, active_log.clone()))
            .encode(&out_of_order)?;
    ...
    assert_v3_binding(admission_failure.cursor(), &expected_binding, 0, 0);
    ...
    let Err(overflow) = LocalLogTailCursorV3::from_trusted_parts(owner, u64::MAX, limits)
    ...
    assert_v3_binding(overflow.cursor(), &returned_binding, u64::MAX, 0);
    ...
    assert_v3_binding(compaction_failure.owner(), &returned_binding, u64::MAX, 0);
    Ok(())
}

#[test]
fn typed_properties_survive_frame_recovery_two_compactions_checkpoint_and_history_replay()
-> TestResult {
    let frame_limits = LocalLogFrameLimits::default();
    let FreshTail { cursor, context, session, active_log } =
        fresh_tail(typed_schema()?, "typed-round-trip", frame_limits)?;
    let initial = cursor.owner().session().state().clone();
    let commit = typed_insertion_commit(&context, &initial, "https://example.test/docs")?;
    let formatted = commit.after().clone();
    let entry = LocalLogEntry::new_with_schema_binding(
        context.schema().durable_binding(),
        session.clone(),
        active_log.clone(),
        LocalLogSequence::FIRST,
        ReplayId::try_new("request:tail-v3:typed")?,
        LocalLogEvent::commit(commit),
    );
    let frame_codec = LocalLogFrameCodecV3::new(
        context.clone(),
        LocalLogFrameBinding::new(session.clone(), active_log.clone()),
    )
    .with_limits(frame_limits);
    let frame = frame_codec.encode(&entry)?;
    let accepted = cursor.try_observe_frame(0, &frame)?;
    assert_eq!(accepted.cursor().owner().session().state(), &formatted);
    assert_eq!(accepted.cursor().owner().session().undo_depth(), 1);
    let (cursor, _) = accepted.into_parts();

    // An exact retry consumes physical bytes but never reapplies typed content.
    let frame_bytes = u64::try_from(frame.len())?;
    let duplicate = cursor.try_observe_frame(frame_bytes, &frame)?;
    assert_eq!(
    ...
    assert_eq!(duplicate.cursor().owner().session().state(), &formatted);
    assert_eq!(duplicate.cursor().owner().session().undo_depth(), 1);
    let (cursor, _) = duplicate.into_parts();

    // Only the property differs: retry equality must compare the complete proof.
    let changed = LocalLogEntry::new_with_schema_binding(
        context.schema().durable_binding(),
        session.clone(),
        active_log.clone(),
        LocalLogSequence::FIRST,
        entry.replay_id().clone(),
        LocalLogEvent::commit(typed_insertion_commit(
            &context,
            &initial,
            "https://example.test/other",
        )?),
    );
    let changed_frame = frame_codec.encode(&changed)?;
    let failure = cursor
        .try_observe_frame(frame_bytes * 2, &changed_frame)
        .err()
        .ok_or("property-only replay conflict was accepted")?;
    assert_eq!(
        failure.error().admission_error().map(crate::local_log::LocalLogRecoveryError::code),
        Some(crate::local_log::LocalLogRecoveryErrorCode::ReplayConflict)
    );
    assert_eq!(failure.cursor().accepted_byte_offset(), frame_bytes * 2);
    assert_eq!(failure.cursor().owner().session().state(), &formatted);
    assert_eq!(failure.rejected_entry(), Some(&changed));
    let (cursor, _, _) = failure.into_parts();

    let successor = LocalLogId::try_new("log:tail-v3:typed-round-trip:g2")?;
    let first = cursor.try_into_checkpoint_anchor(successor.clone())?;
    assert_eq!(first.accepted_prefix_bytes(), frame_bytes * 2);
    let (first_anchor, _, retained_limits, retained_binding) = first.into_parts();
    let first_codec = LocalLogCheckpointJsonCodecV3::new(
        context.clone(),
        LocalLogCheckpointBinding::try_new(session.clone(), active_log, successor.clone())?,
    );
    let first_json = first_codec.encode(&first_anchor)?;
    let first_restored = first_codec.decode(&first_json)?;
    assert_eq!(first_restored.session().state(), &formatted);
    assert_eq!(first_restored.compacted_replay_count(), 1);

    let second_successor = LocalLogId::try_new("log:tail-v3:typed-round-trip:g3")?;
    let second = first_restored
        .begin_successor_tail_v3(LocalLogRecoveryLimits::new(8, 8, 8), retained_limits)
        .try_into_checkpoint_anchor(second_successor.clone())?;
    assert_eq!(second.accepted_prefix_bytes(), 0);
    assert_eq!(second.schema_binding(), &retained_binding);
    let (second_anchor, _, _, _) = second.into_parts();
    let second_codec = LocalLogCheckpointJsonCodecV3::new(
        context,
        LocalLogCheckpointBinding::try_new(session, successor, second_successor)?,
    );
    let second_json = second_codec.encode(&second_anchor)?;
    let second_restored = second_codec.decode(&second_json)?;
    assert_eq!(second_restored.session().state(), &formatted);
    assert_eq!(second_restored.compacted_replay_count(), 1);

    let mut restored_session = second_restored.into_session();
    assert!(restored_session.undo()?.is_some());
    assert_same_semantics(restored_session.state(), &initial);
    assert!(restored_session.redo()?.is_some());
    assert_same_semantics(restored_session.state(), &formatted);
    Ok(())
}
