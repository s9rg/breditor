//! Borrowed deterministic encoding for Editor State V3.

use serde::{Serialize, ser::SerializeStruct};

use crate::{
    record::{PendingFormatRecordV2, SelectionRecordV1, SnapshotIdRecordV1},
...
use super::{
    ...
    editor_state_json_v3::EDITOR_STATE_V3_FORMAT_VERSION,
    editor_value_payload_v1::{encode_selection_record_v1, encode_snapshot_id_v1},
    editor_value_payload_v2::{EditorValueRecordV2Error, encode_pending_format_records_v2},
    schema_binding_encoding::SchemaBindingEncoding,
};

/// Borrowed deterministic encoding of one complete editor state as Editor State V3.
pub(crate) struct EditorStateEncodingV3<'a> {
    ...
    pending_formats: Option<Vec<PendingFormatRecordV2>>,
}

impl<'a> EditorStateEncodingV3<'a> {
    /// Borrows a state for serialization and checks its typed pending formats.
    pub(crate) fn try_new(state: &'a EditorState) -> Result<Self, EditorValueRecordV2Error> {
...
impl Serialize for EditorStateEncodingV3<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut record = serializer.serialize_struct("EditorStateRecordV3", 8)?;
        record.serialize_field("format", EDITOR_STATE_FORMAT)?;
        record.serialize_field("formatVersion", &EDITOR_STATE_V3_FORMAT_VERSION)?;
